...
`.Codex/hooks/verify_landscapes.sh` (a `Stop` hook, registered in
`.Codex/settings.json`) runs `verify_landscape.py --metadata-only` for every
...
(override via `.Codex/hooks/landscapes.txt`). Hooks load at session start; after
editing the hook, restart Codex.
...
2. Add the `selector_for()` case in `.Codex/hooks/verify_landscapes.sh`.
